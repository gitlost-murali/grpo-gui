...
MAX_POSSIBLE_DISTANCE_GUI = math.sqrt(
...
class GUIEvaluator(RewardEvaluator):
    ...
    def _extract_coordinates(self, text: str) -> Optional[Tuple[int, int]]:
        ...
        try:
            ...
            if match:
                ...
                if (
                    0 <= x <= IMAGE_WIDTH * 2 and 0 <= y <= IMAGE_HEIGHT * 2
                ):  # Allow some leeway beyond 224 for robustness
                    return x, y
            return None
    ...
    def _strict_xml_format_reward(
        ...
        for r in responses:
            ...
            if xml_match and coords_extracted:
                ...
                coords_extracted and not xml_match
            ):  # Has x,y but not full XML structure
                rewards.append(
                    0.1
                )  # Small partial credit for at least getting coordinates
            else:
                rewards.append(0.0)
        return rewards
    ...
    def _distance_to_center_reward(
        ...
        for click_xy, target_center_xy, target_bbox in zip(
            extracted_coords, target_centers, target_bboxes
        ):
            ...
            distance = math.sqrt(
                (pred_x - true_center_x) ** 2 + (pred_y - true_center_y) ** 2
            )
            ...
            scaled_reward = max_reward - (
                clamped_distance / MAX_POSSIBLE_DISTANCE_GUI
    ...
    def compute_rewards(
        ...
        ],  # Prompts may not be directly used here but are part of API
        ...
        extracted_coords_list = [
            self._extract_coordinates(comp[0]["content"]) for comp in completions
        ]
        ...
        click_hit_scores = self._click_hit_reward(
            extracted_coords_list, target_bboxes_list, target_names_list
        )

        # 3. Distance to Center Reward
        dist_rewards_scores, raw_distance_errors = self._distance_to_center_reward(
            extracted_coords_list, target_centers_list, target_bboxes_list
        )
        ...
        for i, scores_component in enumerate(all_component_scores):
            rewards_per_func[:, i] = torch.tensor(
                scores_component, dtype=torch.float32, device=device
            )
        ...
        num_hits = sum(
            1 for score in click_hit_scores if score > 0
        )  # count non-zero scores
        click_hit_rate = num_hits / num_completions if num_completions > 0 else 0.0

        # Mean Distance Error
        distance_errors_tensor = torch.tensor(
            raw_distance_errors, dtype=torch.float32, device=device
        )
