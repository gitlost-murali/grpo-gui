...
def generate_correlation_plot(
    r_value, num_points=75, filename="correlation_plot.png", img_size=(224, 224)
):
    """
    ...
    if math.isclose(r_value, 1.0):
        ...
        x = (
            (x_independent - np.mean(x_independent)) / np.std(x_independent)
            if np.std(x_independent) != 0
            else x_independent
        )
        ...
        y = (
            (y_independent - np.mean(y_independent)) / np.std(y_independent)
            if np.std(y_independent) != 0
            else y_independent
        )
    else:
        ...
        x_independent, y_independent = np.random.multivariate_normal(
            mean, cov, num_points
        ).T
        ...
        x = (
            (x_independent - np.mean(x_independent)) / x_std
            if x_std != 0
            else x_independent
        )
        y_prime = (
            (y_independent - np.mean(y_independent)) / y_prime_std
            if y_prime_std != 0
            else y_independent
        )
    ...
    ax.scatter(
        x, y, color="black", marker="s", s=10, zorder=1
    )  # s is marker size, zorder=1 to plot above axes
...
if __name__ == "__main__":
    ...
    print("Finished generating example plots.")
