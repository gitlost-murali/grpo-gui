...
def plot_metric(
    ...
    if claude_score is not None:
        ax.axhline(
            ...
            linestyle="--",
            linewidth=2.5,
            label="Claude Sonnet 3.7 Level",
            zorder=15,
        )  # Gold color, slightly thicker
...
def create_plots():
    ...
    if os.path.exists(claude_eval_json_path):
        try:
            ...
            print(
                f"Error decoding JSON from {claude_eval_json_path}. Claude scores will not be plotted."
            )
        except Exception as e:
            print(
                f"Error loading Claude scores from {claude_eval_json_path}: {e}. Claude scores will not be plotted."
            )
    else:
        print(
            f"Warning: Claude scores file not found at {claude_eval_json_path}. Claude lines will not be plotted."
        )
    ...
    json_files = [
        ...
        if f.startswith("average_scores_round_") and f.endswith(".json")
    ]
    ...
    for i, (metric_name, data) in enumerate(all_metrics_data.items()):
        ...
        if claude_value_for_metric is not None and not isinstance(
            claude_value_for_metric, (int, float)
        ):
            print(
                f"Warning: Claude score for '{metric_name}' is not a number: {claude_value_for_metric}. Not plotting Claude line."
            )
        ...
        plot_metric(
            ...
            output_path,
            current_line_color,
            claude_score=claude_value_for_metric,
        )
...
def plot_metrics(output_dir):
        ...
        for filename in os.listdir(eval_json_dir):
            if filename.startswith("eval_metrics_") and filename.endswith(".json"):
                try:
                    ...
                    print(
                        f"Warning: Could not process evaluation log file {filename}: {e}"
                    )
    else:
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        if not train_logs:
            ...
            reward_metrics = [
                m
                for m in available_train_metrics
                if m.startswith("rewards/") or m == "reward"
            ]
            other_metrics = [
                m
                for m in available_train_metrics
                if m not in reward_metrics and m != "learning_rate"
            ]  # Exclude LR for separate plot
            ...
            metric_color_map = {
                metric: color for metric, color in zip(reward_metrics, colors)
            }
            for metric in reward_metrics:
                color = metric_color_map.get(
                    metric, "#34495e"
                )  # Default color if more metrics than colors
                ...
                for step in steps:
                    ...
                    if metric in train_logs[step_str]:
                        ...
                        print(
                            f"Warning: Metric '{metric}' not found for step {step}. Skipping this point."
                        )

                if not valid_steps:
                    print(
                        f"Warning: No data found for metric '{metric}'. Skipping plot."
                    )
                    plt.close()
                    continue
                ...
                if len(values) >= 5:  # Use >= 5 for moving average calculation
                    ma_values = moving_average(
                        values, window_size=min(5, len(values))
                    )  # Adjust window if fewer points
            ...
            if "learning_rate" in available_train_metrics:
                ...
                plt.figure(figsize=(12, 7))
                lr_values = [
                    train_logs[str(step)]["learning_rate"]
                    for step in steps
                    if "learning_rate" in train_logs[str(step)]
                ]
                valid_lr_steps = [
                    step for step in steps if "learning_rate" in train_logs[str(step)]
                ]
                if valid_lr_steps:
                    plt.plot(
                        valid_lr_steps,
            ...
            metric_color_map = {
                ...
                for metric, color in zip(
                    other_metrics,
                    ["#e67e22", "#9b59b6", "#3498db", "#f1c40f", "#1abc9c"],
                )
            }  # Different colors
        ...
        if eval_logs:
            ...
            if not eval_steps:
                ...
                if expected_key in eval_logs[first_eval_step] and isinstance(
                    eval_logs[first_eval_step][expected_key], dict
                ):
                    # Get all metric keys from the first round's dictionary
                    eval_metric_keys = list(
                        eval_logs[first_eval_step][expected_key].keys()
                    )
                    print(
                        f"Found evaluation metrics in '{expected_key}': {eval_metric_keys}"
                    )

                    # Assign colors to each metric
                    metric_color_map = {
                        metric: color for metric, color in zip(eval_metric_keys, colors)
                    }

                    for metric in eval_metric_keys:
                        color = metric_color_map.get(
                            metric, "#34495e"
                        )  # Default color if not enough colors
                        plt.figure(figsize=(12, 7))
                        ...
                        for step in eval_steps:
                            ...
                            if (
                                expected_key in eval_logs[step]
                                and isinstance(eval_logs[step][expected_key], dict)
                                and metric in eval_logs[step][expected_key]
                            ):
                                metric_values.append(
                                    eval_logs[step][expected_key][metric]
                                )
                                valid_eval_steps.append(step)
                            else:
                                print(
                                    f"Warning: Metric '{metric}' not found in {expected_key} for round {step}. Skipping point."
                                )

                        if not valid_eval_steps:
                            print(
                                f"Warning: No data found for evaluation metric '{metric}'. Skipping plot."
                            )
                            plt.close()
                            continue
                    ...
                    print(
                        f"Warning: '{expected_key}' key missing or not a dictionary in first evaluation log (round {first_eval_step}). Skipping evaluation metric plots."
                    )
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot training and evaluation metrics from logs directory"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        required=True,
        help="Main output directory containing training_logs and eval_logs subdirectories",
    )
